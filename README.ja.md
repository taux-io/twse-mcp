...
上場企業の概要（PER・配当利回り・月次売上・財務諸表・コーポレートガバナンス）、
PER や利回りでの絞り込み、市場概況と先物の建玉動向、
...
2. **URL を貼り付ける** — **「＋ カスタムコネクタを追加」** をクリックします。
   ...
   **Authentication（認証）** は **No sign-in** を選びます（通常は自動で選ばれ、
   「Detected」と表示されます）。URL を知っていれば誰でも使えるという黄色の注意が
   出ますが、このサービスは公開なので問題ありません。**Request headers は追加不要**です。
