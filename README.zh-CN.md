...
包装成一个**远程 MCP 服务器**，让 AI 用大白话就能查台股行情、上市公司概况（本益比、殖利率、月营收、财报、公司治理）、
按本益比或殖利率筛选排序、大盘概况与期货筹码、ETF 资料、期货与期权每日行情，以及两家交易所公开的两百多种报表。
...
2. **粘贴网址** — 点 **"+ 添加自定义连接器"**。**名称**随便取（例如 `台股`），
   **网址**贴下面这一行（鼠标移到框上，右上角会出现复制按钮）：

   ```
   https://twse-mcp.taux.io/mcp
   ```

   **Authentication（验证方式）** 选 **No sign-in**——通常已经自动选好，旁边会标
   「Detected」。下面会出现黄色提醒说任何人拿到网址都能用；这个服务本来就是公开的，
   可以放心略过。**Request headers 不用加**，这个服务没有 API key。
...
除了直接用大白话问，服务还内建三个快捷命令，会**跟着连接器自动出现**，不用另外装东西。

| 命令 | 做什么 | 传什么 |
|---|---|---|
| `find_dataset` | 不知道该查哪张表时，用关键词找出对的那一个 | 关键词，例如 `三大法人` |
| `etf_overview` | 一次看完一只上市 ETF 的基本资料、前一交易日价量和定投热度 | ETF 代码，例如 `0056` |
| `futures_quote` | 查期货或期权的每日行情 | 合约代码或商品名，例如 `TX` |

在 **Claude Desktop**：点输入框旁边的 **"+"**，在菜单里找到它们。
在 **Claude Code**：输入 `/` 就会列出来，格式是 `/mcp__twse__find_dataset`。

不用也没关系——它们只是把常见问法先写好了，直接用中文问一样有效。
