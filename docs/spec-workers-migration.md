...
**MCP 傳輸與 handler**
- 用 **`createMcpHandler()`（stateless）**，端點 `/mcp`，**不使用 Durable Objects**。理由：工具全為唯讀、無跨呼叫 session 狀態（v1 時 5 個；2026-09 擴充到 8 個後仍全為唯讀）。
...
- **成本**：Workers 與 Cache API 免費額度對此用量綽綽有餘，預期 $0。
  **帳戶在 Workers 免費方案（2026-09-26 確認）。** 當天的效能實測（見下一條）量到單次請求最高 319 ms CPU，
  遠超過一般認知的免費方案每請求 10 ms 上限，但**所有工具呼叫都成功**。同一天查 Workers Observability 的
  24 小時紀錄：5,814 筆請求中只有 9 筆 `outcome: exceededCpu`，**全部是 `subscriptions/listen` 長連線**
  （Claude 用戶端開著約 8.5 小時、CPU 累積到 297–433 ms），且同時在 v0.7.0 部署時結束；用戶端會自動重連。
  沒有任何查詢請求被 CPU 上限中斷。
  **判讀**：目前的用量與負載在免費方案下沒有造成使用者可見的失敗。若日後查詢請求本身開始出現
  `exceededCpu`，兩個選項：升級 Workers Paid（每月 5 美元），或做「快取解析後的列資料」降低 CPU。
