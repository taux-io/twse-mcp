...
물어볼 수 있게 해주는 **원격 MCP 서버**입니다. 대만 주식 시세, 상장사 개요(PER·배당수익률·월매출·재무제표·지배구조),
PER·수익률 기준 종목 필터링, 시장 개황과 선물 포지션, ETF 정보, 선물·옵션 일일 시세, 두 거래소가 공개하는 200종 이상의 보고서를 다룹니다.
...
2. **URL 붙여넣기** — **"+ 사용자 지정 커넥터 추가"**를 누릅니다.
   ...
   **Authentication(인증)** 은 **No sign-in** 을 고릅니다(보통 자동으로 선택되어
   "Detected"라고 표시됩니다). URL만 알면 누구나 쓸 수 있다는 노란 안내가 나오지만,
   이 서비스는 원래 공개라서 괜찮습니다. **Request headers는 추가하지 않습니다.**
